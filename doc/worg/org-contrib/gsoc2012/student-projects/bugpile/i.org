...
# To introduce classes and inheritance in the iOrg framework, we make
# use of the attribute inheritance of Org-mode:
...
# Attribute inheritance makes it possible to set attribute values at the
# class level and let all instantiated objects inherit the same value.
# However, it may slow down property searches:
...
# drawer of the object prototype entry (the last node in the outline
# hierarchy of a new class), maintaining their order. 
...
the permissions into the users, but rather define /roles/ with certain
permissions and then assign the /users/ to one or more roles.
...
If we want to instantiate object of these classes, we have to create a
=/objects/<<class-name>>-obj.org= file for each of the classes and add
one top-level entry for every newly instantiated object:
...
One could say that the superclass attributes are 'injected' into each
objects attribute drawer. 

# Using /speed inheritance/ to speed up the attribute search, we
...
# case, a solution based on subclassing is too complicated for the
