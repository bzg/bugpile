...
the well known MVC (Model-View-Controller) architecture for web
applications. 
...
#+begin_src plantuml :file ../../../../images/gsoc/2012/bugpile/refined-arch-iorg.png 
    ...
    package Objects_DIR {
          ...
          package Object3_FILE {}
    }
  

  UseCase1_DIR +-- UseCase2_DIR
  UseCase2_DIR +-- UseCaseIndependentComponents_DIR
  View_DIR +-down- Controller_DIR
  Controller_DIR +-down- Logic_DIR
  Logic_DIR  +-down- Objects_DIR
...
case independent components. The application objects (one file per
object) are stored in the /objects/ directory. 
...
one .org file in the /objects/ directory. Relations with other objects
...
one level. The following function is used to =get= the properties:
...
#+begin_quote
Org mode does not turn [attribute inheritance] on by default, because
...
trees with attribute inheritance (/speed inheritance/). It simply
...
let subclasses =white-car=, =green-car= and =black-car= inherit from
superclass =car=). 
...
are three types of users, /user/, /assignee/ and /admin/. Since a
...
are allowed to do in the system, in their /permissions/.

Following the example of well-established software systems, we don't
hardcode the permissions into the users, but rather define /roles/
with certain /permissions/ and then assign the /users/ to one or more
roles.  
...
#+srcname bugpile-users-class-model-subclass
#+begin_src plantuml :file ../../../../images/gsoc/2012/bugpile/bugpile-users-class-model-subclass.png 
  title Bugpile Users (via Subclasses)
  User : name\nemail
  Assignee : assigneePermissions
...
#+results:
[[file:../../../../images/gsoc/2012/bugpile/bugpile-users-class-model-subclass.png]]

#+srcname bugpile-users-class-model-attr
#+begin_src plantuml :file ../../../../images/gsoc/2012/bugpile/bugpile-users-class-model-attr.png 
  title Bugpile Users (via Attributes)
  User : name\nemail
  Role : name 
  Permission : name
  User "             0..*" -down- "1..*           " Role
...
#+results:
[[file:../../../../images/gsoc/2012/bugpile/bugpile-users-class-model-attr.png]]


When using /subclassing/, we need three classes and one object for every user
...
#+begin_src org :exports source
  ...
  ,  : PROPERTIES:
  ,  : super:    user
  ,  : assigneePermissions_ALL: reopen_task edit_task close_task
  ,  : assigneePermissions: <<permission>>
  ,  : END:
#+end_src
...
#+begin_src org :exports source
  ...
  ,  :PROPERTIES:
  ,  :super:    assignee
  ,  :adminPermissions_ALL: create_project edit_project add_assignee assign_assignee
  ,  :adminPermissions: <<permission>>
  ,  :END:
#+end_src

If we want to instantiate a new /admin/, we have to create a new admin
...
#+begin_src org :exports source
  ...
  ,   :super:    user
  ,   :assigneePermissions_ALL: reopen_task edit_task close_task
  ,   :assigneePermissions: edit_task
  ,   :END:
  ,*** admin
  ,  :PROPERTIES:
  ,  :super:    assignee
  ,  :adminPermissions_ALL: create_project edit_project add_assignee assign_assignee
  ,  :adminPermissions: edit_project
  ,  :adminPermissions+: create_project
  ,  :END:
#+end_src
...
tasks. Using /speed inheritance/ to speed up the attribute search, we
would simply move all the attributes from the parent entries into the
attribute drawer of the object entry:

#+begin_src org :exports source
  ,# object admin henry (with 'speed-inheritance')
  ,* user
  ,  :PROPERTIES:
  ,  :super:    root
  ,  :END:
...
another attribute =role= in the /user/ class and discriminate the
different user types by the value of this attribute. We define the
class =role= that declares all possible roles in the system. Another
class =permissions= assigns permissions to each of the roles.
...
#+begin_src org :exports source
  ...
  ,  :super:    root
  ,  :user:     unpriviledged
  ,  :assignee: close_task reopen_task edit_task self_assign_task
  ,  :admin:    create_project edit_project search_user action_on_user assign_task
  ,  :END:
#+end_src
...
#+begin_src org :exports source
  ...
  ,  :PROPERTIES:
  ,  :super:    root
  ,  :role_ALL: user assignee admin
  ,  :END:
#+end_src

Then we add the =role= attribute to the =user= class. 
...
To instantiate user object 'Henry' with role /admin/ we only have
to 'inject' one superclass =role= and assign the value
'admin' to the attribute =role=. 
...
/note: we use subclassing mostly to achieve attribute inheritance, we/
/don't intend to establish class hierarchies based on specialisation/
/and generalisation. a user is not a specialisation of a role, but it/
/is convenient to declare 'role' as a superclass of 'user' to inherit the/
/role_ALL definition./


*** Software Oriented Activities (SOA)
*** Complex System Activities
**** Search Tasks
    :PROPERTIES:
